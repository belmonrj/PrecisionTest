...
// <cos(n(phi1+phi2))>
float PrecisionTest::calccossum2_event(TComplex& Qn, TComplex& Q2n, float M)
{
  if ( M < 2 ) return -9999;
...
// <sin(n(phi1+phi2))>
float PrecisionTest::calcsinsum2_event(TComplex& Qn, TComplex& Q2n, float M)
{
  if ( M < 2 ) return -9999;
  TComplex result = Qn*Qn - Q2n;
...
float PrecisionTest::calccos3_event(TComplex& Qn, TComplex& Q2n, float M)
{
  if ( M < 3 ) return -9999;
...
float PrecisionTest::calcsin3_event(TComplex& Qn, TComplex& Q2n, float M)
{
  if ( M < 3 ) return -9999;
  TComplex result = Qn*TComplex::Conjugate(Qn)*TComplex::Conjugate(Qn) - Qn*TComplex::Conjugate(Q2n);
...
float PrecisionTest::calc6_event(TComplex& qn, TComplex& q2n, TComplex& q3n, float M)
{
...
TComplex PrecisionTest::Recursion(int n, int* harmonic)
{
  return Recursion(n,harmonic,1,0); // 1 and 0 are defaults from above
}

TComplex PrecisionTest::Recursion(int n, int* harmonic, int mult, int skip)
{
...
TComplex PrecisionTest::Q(int n, int p)
{
...
// --- from generic forumulas but now using long double complex ----------------------------------------------------
LDComplex PrecisionTest::LDRecursion(int n, int* harmonic)
{
  return LDRecursion(n,harmonic,1,0); // 1 and 0 are defaults from above
}

LDComplex PrecisionTest::LDRecursion(int n, int* harmonic, int mult, int skip)
{
 // Calculate multi-particle correlators by using recursion (an improved faster version) originally developed by
 // Kristjan Gulbrandsen (gulbrand@nbi.dk).

  int nm1 = n-1;
  LDComplex c(LDQ(harmonic[nm1], mult));
  if (nm1 == 0) return c;
  c *= LDRecursion(nm1, harmonic);
  if (nm1 == skip) return c;
  ...
  harmonic[counter1] = harmonic[nm2];
  harmonic[nm2] = hhold + harmonic[nm1];
  LDComplex c2(LDRecursion(nm1, harmonic, multp1, nm2));
  int counter2 = n-3;
  while (counter2 >= skip) {
    harmonic[nm2] = harmonic[counter1];
    harmonic[counter1] = hhold;
    ...
    harmonic[counter1] = harmonic[nm2];
    harmonic[nm2] = hhold + harmonic[nm1];
    c2 += LDRecursion(nm1, harmonic, multp1, counter2);
    --counter2;
  }
  harmonic[nm2] = harmonic[counter1];
  harmonic[counter1] = hhold;
  ...
  if (mult == 1) return c-c2;
  return c-(long double)mult*c2;

}

LDComplex PrecisionTest::LDQ(int n, int p)
{
  // Using the fact that Q{-n,p} = Q{n,p}^*.
  if(n>=0){return LDQvector[n][p];}
  return LDComplex::Conjugate(LDQvector[-n][p]);
} // LDComplex Q(int n, int p)
// -------------------------------------------------------------------------------
